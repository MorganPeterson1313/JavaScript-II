...
// fullName.push(runners.forEach(value => `${value.first_name}, ${value.last_name}`));
...
fullName.map(function(array){
    allCaps.push(`${array}`.toLocaleUpperCase());
})
console.log(allCaps); 
...
//     }    
//               return largeShirts.push(`${array.first_name}`);
    

   
// });
largeShirts.push(runners.filter( runners => runners.shirt_size === "L" ))
...
runners.forEach(function(array){ 
    ticketPriceTotal.push(array.donation);
});
